...
### 선택 필드
- [ ] `stage`: `"기초 실습" | "실습 응용" | "심화"` — ch02/03/04/05/06/07에서 ClipTabs 뱃지로 표시 (ch01 제외)
- [ ] `period`: `1 | 2 | 3 | 4` — 교시 매핑 (ch01·ch02=1교시 / ch03·ch04=2교시 / ch05=3교시 / ch06·ch07=4교시). PR-A 회의 결정 적용. ch08 archive 미적용.
...
PR9 A3에서 도입 → PR12 followup에서 ch03·ch07 추가. **본편 7챕터 중 6/7 적용** (ch01만 자유).
...
- **ch05 실행하기** (4 clips)
- **ch06 설득하기** (4 clips)
- **ch07 축적하기** (3 clips, DEEP DIVE 마지막)

### 비적용 챕터
- **ch01 사전 준비** — 체크리스트 성격, 환경 세팅 + Canvas + 딥리서치 도구가 독립. 챕터 짧고 "심화"가 자연 부재
- **ch08 치트시트** — 참고 자료 모음 (학습 단계 단위가 아닌 도구 사전)
...
### DEEP DIVE clip의 stage 처리 (PR-B 결정)
- `deepDive: true` clip은 **챕터의 마지막 위치**에 배치 (ux 권고: self-paced "더 깊이 보기" 토글 패턴)
- DEEP DIVE clip의 stage = `"심화"`로 일관 적용
- ClipTabs에서 `stage` 뱃지와 `DEEP` 뱃지가 같이 표시되어 "심화 + 별도 게이트" 의미 명확
...
## 7. 교시(period) 매핑 (PR-A 회의 결정)

`curriculum.json`의 chapter `period` 필드 기준 + GNB·홈 카드에 라벨 표시.

### 매핑
- **1교시** = ch01 사전 준비 + ch02 듣기
- **2교시** = ch03 말하기 + ch04 질문하기
- **3교시** = ch05 실행하기 (단독 = "집중 실행 세션")
- **4교시** = ch06 설득하기 + ch07 축적하기

### 적용 위치
- **GNB**: TimelineNav에서 dot 7개 사이에 period 변경 시 separator + "N교시" 라벨
- **홈 chapter 카드**: 각 교시 첫 chapter 앞 separator + 테마/도구 ecosystem 라벨 (`PERIOD_THEMES` in page.tsx)
- **챕터 카드 헤더**: "N교시 · JB의 HH:MM · timeLabel" 시간 병기 (윤서영 권고)

### JB 시나리오 ↔ 교시 두 레이어 분리 (ux 권고)
- **시계 시간** (07:00~17:00): 정서 곡선·Dayscape에서 그대로 유지
- **교시 라벨** (1·2·3·4교시): GNB · 챕터 카드 · ClipTabs에서 사용
- 두 레이어는 충돌 없이 공존. 시각 시간은 JB 서사를, 교시는 운영 단위를 표현.

### 챕터 subtitle 패턴 (PR-B 결정)
- 형식: `"챕터명 — JB가 N교시에 ___을 한다"`
- 예: `"사전 준비 — JB가 1교시 시작 전에 도구를 세팅한다"`
- 1인칭 narrative로 교시 + 시나리오 통합 표현

---

## 8. 5대 기업 조직문화 패턴 (PR-B 회의 결정)

Hero 약속 "경영진 보고서는 5대 기업(삼성·현대·SK·LG·롯데) 조직문화 분석 기반"의 실현 흐름.

### 도입 위치
- **ch06/clip01** (벤치마크 수집): 5대 기업 조직문화 딥리서치 프롬프트
- **ch06/clip02** (보고서 뼈대): 위 결과물을 입력 자료로 사용 — important 박스로 명시

### 신규 기업 강의 재활용 (마케터 권고)
- 5대 기업 = 공개 정보 기반 → 어느 기업 수강생에게도 적용 가능
- 기업 특화 콘텐츠(예: LG 이모티콘)는 **사이트 X**, PDF·패들렛에서만 처리
- 사이트 = 기업 중립 / PDF = 기업 특화 (마케터 권고 일관)

---

## 9. 페르소나 리뷰 사이클 (PR 반영 시)
